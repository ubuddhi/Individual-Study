...
object SparkNaiveBayes {
  ...
  def main(args: Array[String]) {
    ...
    sc.parallelize(data.select("cont_name", "explode").repartition(1).collect().toSeq).saveAsTextFile("result_nb.txt")
    ...
    sc.parallelize(predictions.select("predictedLabel", "explode", "cont").repartition(1).collect().toSeq).saveAsTextFile("nb_prediction.txt")
